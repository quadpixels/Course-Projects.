...
//#define BW 4				//Commnted by Manish. conflicts with something else
#ifndef ucharBV
#define ucharBV unsigned char
#endif
#ifndef ulongBV
#define ulongBV unsigned long
#endif
...
inline ulongBV GetField(ulongBV *A, register  ulongBV len, register ulongBV index) {
  register ulongBV i=index*len/W, j=index*len-W*i, result;
  if (j+len <= W)
...
inline void SetField(ulongBV *A,register ulongBV len, register ulongBV index,register  ulongBV x) {
   ulongBV i=index*len/W, j=index*len-i*W;
   ulongBV mask = ((j+len) < W ? ~0u << (j+len) : 0) | ((W-j) < W ? ~0u >> (W-j) : 0);
   A[i] = (A[i] & mask) | x << j;
   if (j+len>W) {
      mask = ((~0u) << (len+j-W));
...
inline int len_uchar(ucharBV *s) {
  int i;
  for (i=0;;i++) 
    if (s[i] == '\0') return i;
}
